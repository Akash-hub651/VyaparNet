...
export function KYCTab() {
  ...
  const currentKycStatus = safeUser?.business?.kycStatus || 'NOT_SUBMITTED';
  const isKycPendingOrVerified = currentKycStatus === 'PENDING' || currentKycStatus === 'VERIFIED';
  ...
  const handleUploadSuccess = (docId: string, fileData: { id: string, filename: string, url: string, sizeBytes: number }) => {
  ...
  const handleSubmit = async () => {
    ...
    const allRequiredUploaded = documents.every(d => !d.required || d.status === 'uploaded');
    if (!allRequiredUploaded) {
      addToast({ message: "Kripya sabhi required documents upload karein.", variant: "error" });
    ...
    try {
      ...
      if (!res.success) {
        addToast({ message: "KYC submit nahi ho paya. " + res.error, variant: 'error' });
      } else {
        addToast({ message: "KYC documents submit ho gaye! 24-48 ghante mein verify hoga.", variant: 'success' });
        // NOTE: In a real app, we would mutate the `user` context here or force a reload to update `kycStatus` to 'PENDING'.
        // For simulation, we reload the page or documents
        setTimeout(() => {
          window.location.reload();
        }, 1500);
      }
    } catch {
      addToast({ message: "Server error during KYC submission.", variant: 'error' });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className="animate-in fade-in max-w-4xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          {[1, 2, 3].map(i => (
            <div key={i} className="w-full h-24 bg-neutral-100 rounded-lg border border-neutral-200 animate-pulse"></div>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 md:pb-0 animate-in fade-in duration-300">
      ...
      <KycStatusCard 
        ...
        onScrollToDocs={() => {
          document.getElementById('kyc-docs-section')?.scrollIntoView({ behavior: 'smooth' });
        }}
      />

      <div id="kyc-docs-section" className="mt-8">
        <h3 className="text-base font-semibold text-text-primary">Required Documents</h3>
        <p className="text-xs text-text-secondary mt-1 mb-6">
          Sab required documents upload karein. Note: File simulation is active for Sprint 8.
        </p>
        ...
        {isError ? (
          ...
          <div className="w-full border border-border-default rounded-lg p-8 bg-surface-card text-center flex flex-col items-center">
            ...
            <h4 className="text-sm font-semibold text-text-primary mb-1">KYC Requirements Unavailable</h4>
            <p className="text-xs text-text-secondary max-w-sm mb-4">
              Hum abhi required documents ki list load nahi kar paaye. Common business verification documents mein <strong>GST Certificate</strong> aur <strong>PAN Card</strong> aate hain, par kripya list load hone tak wait karein.
            </p>
            <button 
          ...
          <div className="w-full border border-border-default border-dashed rounded-lg p-8 bg-neutral-50 text-center">
            <p className="text-sm text-text-secondary">Koi document requirement list nahi mili.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {documents.map(doc => (
              <DocumentUploadCard 
                key={doc.id} 
                doc={doc} 
                onPreview={(url, filename, type) => setPreviewFile({ url, filename, type })}
        ...
        {!isError && documents.length > 0 && currentKycStatus !== 'PENDING' && currentKycStatus !== 'VERIFIED' && (
          <div className="mt-8 border-t border-border-default pt-6">
            <button
              ...
              title={!allRequiredUploaded ? "Sab required documents upload karein" : ""}
            >
              {isSubmitting ? (
            ...
            {!allRequiredUploaded && (
              <p className="text-xs text-center text-text-muted mt-3">
                Button enable karne ke liye sabhi *Zaroori documents upload karein
              </p>
            )}
          </div>
        )}
