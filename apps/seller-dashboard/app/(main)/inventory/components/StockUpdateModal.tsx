    ...
    >
      <form onSubmit={handleSubmit} className="p-1">
        ...
        <div className="bg-neutral-50 rounded-lg p-3 flex items-center gap-3 mb-6 border border-neutral-200">
          <div className="w-10 h-10 rounded bg-white border border-neutral-200 overflow-hidden relative flex-shrink-0">
            {product.productImage ? (
              <Image src={product.productImage} alt="Product" fill className="object-cover" sizes="40px" />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-text-primary truncate">{product.productName}</p>
            <p className="text-xs text-text-muted font-mono truncate">{product.productSku}</p>
          </div>
          <div className="text-right">
             <p className="text-xs text-text-secondary">Current</p>
             <p className="text-sm font-semibold text-text-primary">{currentStock} {product.unit}</p>
          </div>
        </div>

        {error && (
          <div className="mb-4 bg-error-50 border-l-4 border-error-500 p-3 rounded-r-md" role="alert">
        ...
        <fieldset className="space-y-5">
          {/* Update Type Radios */}
          <div>
            <legend className="text-sm font-medium text-text-primary mb-2">Action</legend>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <label className={`flex items-center gap-2 p-3 border rounded-lg cursor-pointer transition-colors ${updateType === 'add' ? 'border-brand-500 bg-brand-50' : 'border-neutral-200 hover:bg-neutral-50'}`}>
                <input type="radio" name="updateType" value="add" checked={updateType === 'add'} onChange={() => setUpdateType('add')} className="text-brand-600 focus:ring-brand-500" />
                <span className="text-sm font-medium text-text-primary leading-tight">Add<br/><span className="text-xs text-text-muted font-normal">stock mein add</span></span>
              </label>
              <label className={`flex items-center gap-2 p-3 border rounded-lg cursor-pointer transition-colors ${updateType === 'set' ? 'border-brand-500 bg-brand-50' : 'border-neutral-200 hover:bg-neutral-50'}`}>
                <input type="radio" name="updateType" value="set" checked={updateType === 'set'} onChange={() => setUpdateType('set')} className="text-brand-600 focus:ring-brand-500" />
                <span className="text-sm font-medium text-text-primary leading-tight">Set<br/><span className="text-xs text-text-muted font-normal">exact quantity set</span></span>
              </label>
              <label className={`flex items-center gap-2 p-3 border rounded-lg cursor-pointer transition-colors ${updateType === 'remove' ? 'border-error-500 bg-error-50' : 'border-neutral-200 hover:bg-neutral-50'}`}>
                <input type="radio" name="updateType" value="remove" checked={updateType === 'remove'} onChange={() => setUpdateType('remove')} className="text-error-600 focus:ring-error-500" />
                <span className="text-sm font-medium text-text-primary leading-tight">Remove<br/><span className="text-xs text-text-muted font-normal">stock se nikaalein</span></span>
              </label>
            </div>
          </div>
