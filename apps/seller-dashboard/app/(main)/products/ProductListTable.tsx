...
import { Edit2, MoreVertical, Archive, Play, RefreshCw, AlertCircle } from 'lucide-react';
import { ProductStatus } from '@vyaparnet/types';
import { type ProductResponse, archiveProduct, publishProduct, restoreProduct } from '../../../lib/api/products.client';
...
export function ProductListTable({
  ...
  const handleAction = async (action: 'archive' | 'publish' | 'restore', product: ProductResponse) => {
    if (!accessToken || isStaff) return;
    
    if (action === 'archive') {
      if (!confirm(`Ye product archive ho jayega. Buyers isay nahi dekh payenge. Confirm karein?`)) return;
    }

    try {
      let res;
      if (action === 'archive') res = await archiveProduct(product.id, accessToken);
      else if (action === 'publish') res = await publishProduct(product.id, accessToken);
      else if (action === 'restore') res = await restoreProduct(product.id, accessToken);
      ...
      addToast({ message: 'Action fail ho gaya. Dobara try karein.', variant: 'error' });
    }
  };
            ...
            {products.map((product) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-start gap-3">
                      {firstImage ? (
                        <img 
                          src={firstImage} 
                          alt={product.name} 
                          className="w-10 h-10 rounded-md object-cover flex-shrink-0 bg-surface-card" 
                        />
                      ) : (
                        <div className="w-10 h-10 rounded-md bg-brand-600 text-white flex items-center justify-center font-semibold text-xs flex-shrink-0">
                          {initials}
                        </div>
                      )}
                      <div>
                  ...
                  <td className="px-4 py-4 text-right align-top pt-5 whitespace-nowrap">
                    <div className={`text-sm font-mono ${product.moq === 0 ? 'text-error-700 font-semibold' : 'text-text-primary'}`}>
                      {product.moq} pcs
                    </div>
                    {product.moq === 0 && (
                  ...
                  <td className="px-4 py-4 align-top pt-4">
                    ...
                    {isRejected && product.rejectionReason && (
                      <div className="text-xs text-error-600 mt-1 line-clamp-1 max-w-[140px]" title={product.rejectionReason}>
                          ...
                          <div className="absolute right-0 top-full mt-1 w-48 bg-surface-default border border-border-default rounded-md shadow-2 opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10 text-left">
                            <div className="py-1">
                              {product.status === ProductStatus.DRAFT && (
                                ...
                                >
                                  <Play size={14} className="text-brand-600" /> Publish
                                </button>
                              )}
                              
                              {(product.status === ProductStatus.ACTIVE || isRejected) && (
                                <button
                              ...
                              {isArchived && (
                                <button
                                  onClick={() => handleAction('restore', product)}
                                  className="w-full px-4 py-2 text-sm text-left flex items-center gap-2 hover:bg-surface-hover text-text-primary min-h-[44px]"
                                >
                                  <RefreshCw size={14} className="text-brand-600" /> Restore
                                </button>
                              )}

                              {isRejected && (
                                <button
                                  onClick={() => alert(`Rejection reason: ${product.rejectionReason ?? 'N/A'}`)}
