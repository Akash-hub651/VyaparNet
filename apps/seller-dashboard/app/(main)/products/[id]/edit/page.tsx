...
import { getCategories, CategoryResponse, getSegmentAttributeSchema } from '../../../../../lib/api/categories.client';
...
export default function ProductEditPage({ params }: { params: { id: string } }) {
  ...
  const draftKey = businessId ? `${DRAFT_PREFIX}${params.id}-${businessId}` : null;
  ...
  const isDirty = useMemo(() => {
    ...
    const originalData = {
      ...
      mediaIds: product.media?.map((m: any) => m.mediaId) || [],
  ...
  useEffect(() => {
    ...
    void getCategories(undefined, accessToken).then((res: any) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        const initialDraft: DraftData = {
          ...
          tags: p.tags.join(', '),
          segmentAttributes: (p.segmentAttributes as Record<string, string>) || {},
          ...
          media: p.media ? p.media.map((m: any) => ({
            mediaId: m.mediaId,
            url: m.url,
            mediaClass: m.mediaClass === 'SWATCH' ? 'SWATCH' : 'PRODUCT_IMAGE',
          ...
          try {
            ...
            if (saved) {
              ...
              if (age < 7 * 24 * 60 * 60 * 1000) {
                 ...
                 (window as any).__storedDraft = parsed;
              }
            }
          } catch {
  ...
  useEffect(() => {
    if (isStaff && !loading) {
      addToast({ message: 'Products edit karne ki permission nahi hai', variant: 'error' });
  ...
  // -- Handlers --
  const updateDraft = (key: keyof DraftData, value: any) => {
  ...
  const handleRestoreDraft = () => {
    const saved = (window as any).__storedDraft;
    if (saved) {
      setDraft(saved);
  ...
  const validateStep1 = () => {
    ...
    if (!draft.name || draft.name.length < 3) errs['name'] = 'Naam kam se kam 3 characters ka hona chahiye';
    if (!draft.categoryId) errs['categoryId'] = 'Segment select karein';
    if (!draft.description || draft.description.length < 50) errs['description'] = 'Description mein kam se kam 50 characters likhein';
    
    setErrors(errs);
    if (Object.keys(errs).length > 0) {
      addToast({ message: 'Pehle zaroori fields poori karein.', variant: 'error' });
      return false;
    }
    return true;
  };
  ...
  const validateStep2 = () => {
    const errs: Record<string, string> = {};
    if (!draft.basePrice || Number(draft.basePrice) <= 0) errs['basePrice'] = 'Price 0 se zyada hona chahiye';
  ...
  const handleSubmit = async (targetStatus?: 'DRAFT' | 'PENDING_REVIEW') => {
    ...
    if (product.status === 'ACTIVE' && !ackChecked) {
      addToast({ message: 'Bina acknowledgement check kiye save nahi kar sakte.', variant: 'error' });
      return;
    }
    ...
    if (finalStatus !== 'DRAFT' && draft.media.length === 0) {
      addToast({ message: 'Kam se kam 1 photo zaroori hai (Draft chhod ke).', variant: 'error' });
      return;
    }
    ...
    try {
      ...
      if (res.success) {
         ...
         addToast({ message: 'Product successfully update ho gaya!', variant: 'success' });
         ...
         addToast({ message: res.error || 'Changes save nahi ho payi.', variant: 'error' });
      }
    } catch {
       addToast({ message: 'Network error. Changes save nahi ho payi.', variant: 'error' });
    } finally {
     ...
     return (
       <div className="max-w-2xl mx-auto pb-12 px-4 mt-8">
         <div className="bg-surface-card border border-warning-200 rounded-xl p-8 flex flex-col items-center text-center">
           ...
           <h2 className="text-xl font-semibold text-warning-700 mb-2">Product Review Mein Hai</h2>
           <p className="text-sm text-text-secondary max-w-md mb-6">
             Admin {product.name} ko review kar raha hai. Ye process 24-48 ghante leta hai. Jab tak review complete na ho, aap edit nahi kar sakte.
           </p>
  ...
  const parseRejectionReasons = (reasonStr: string) => {
     if (!reasonStr) return [];
     return reasonStr.split(/(?:\r?\n|;)+/).map(s => s.trim()).filter(Boolean);
  };

  return (
    <div className="max-w-2xl mx-auto pb-12 px-4 mt-4">
      <div className="sr-only" aria-live="polite">
        Step {step} of 3: {step === 1 ? 'Basic Info' : step === 2 ? 'Pricing & Stock' : 'Images & Publish'}
      </div>
        ...
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-lg p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-amber-800">
              ...
              (saved: {formatRelativeTime(new Date(draftTimestamp).toISOString())})
            </p>
          </div>
      ...
      {product.status === 'ACTIVE' && (
        <div className="mb-6 bg-warning-50 border-l-4 border-warning-500 p-4 rounded-r-lg" role="alert">
          <div className="flex gap-3">
             ...
             <div>
                ...
                <p className="text-xs text-warning-700 mt-1">
                  Changes save karne ke baad ye temporarily unlisted ho jayega jab tak admin re-review na kare (24-48 hrs).
                </p>
                <div className="mt-3 flex items-start gap-2">
                   <input 
                      ...
                      aria-required="true"
                   />
                   <label htmlFor="ack-active" className="text-sm text-warning-700 select-none">
      ...
      {product.status === 'ARCHIVED' && (
         <div className="mb-6 bg-neutral-50 border-l-4 border-neutral-400 p-4 rounded-r-lg" role="alert">
           <div className="flex gap-3">
              <Archive className="w-5 h-5 text-neutral-400 flex-shrink-0" />
              <div>
                 <p className="text-sm font-medium text-neutral-700">Ye product archived hai — buyers ise nahi dekh sakte.</p>
                 <p className="text-xs text-neutral-500 mt-1">
                   Yahan changes save karne ke baad product DRAFT mein chala jayega. Phir aap ise review ke liye submit kar sakte hain.
                 </p>
              </div>
           </div>
         </div>
      ...
      {product.status === 'REJECTED' && (
         <div className="mb-6 bg-error-50 border border-error-200 rounded-lg p-4" role="region" aria-label="Rejection reasons">
           <div className="flex gap-3">
              <XCircle className="w-5 h-5 text-error-500 flex-shrink-0" />
              <div className="w-full">
                 <h3 className="text-sm font-bold text-error-700">Product Reject Ho Gaya</h3>
                 <p className="text-xs text-error-600 mt-0.5">Buyers is product ko nahi dekh pa rahe. Neeche reasons fix karein.</p>
                 
                 <ul className="mt-3 space-y-1" role="list">
                   {parseRejectionReasons(product.rejectionReason || 'No reason provided').map((r, i) => (
                     <li key={i} className="text-sm text-error-700 flex items-start gap-2" role="listitem">
          ...
          <React.Fragment key={num}>
            ...
            >
              ...
              <span
                ...
                `}
              >
                {num === 1 ? 'Basic Info' : num === 2 ? 'Pricing & Stock' : 'Images'}
              </span>
            </div>
            {i < 2 && (
              <div
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-3 mt-8 pt-6 border-t border-neutral-100">
          <button
            ...
            className={`w-full sm:w-auto px-6 py-2.5 text-sm font-semibold rounded-lg transition-colors min-h-[44px] ${
              step === 1 ? 'invisible' : 'text-neutral-600 hover:bg-neutral-100 active:bg-neutral-200'
            }`}
          >
            ← Pichha
          </button>
            ...
            <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-3">
              ...
              {product.status === 'REJECTED' && (
                 <div className="flex flex-col gap-1 w-full sm:w-auto">
                   ...
                   <span className="text-center sm:text-right text-xs text-text-muted mt-1">Rejection fix karke submit karein</span>
                 </div>
              )}
              ...
              {product.status === 'ARCHIVED' && (
                 <div className="flex flex-col gap-1 w-full sm:w-auto">
                   ...
                   <span className="text-center sm:text-right text-xs text-text-muted mt-1">Save karne ke baad product DRAFT mein jayega</span>
                 </div>
              )}
            </div>
          )}
