...
import { getSellerRfqs, adaptRfqToViewModel, RfqViewModel, RfqStatus } from '../../../lib/api/rfq.client';
...
export default function RfqListPage() {
  ...
  const filteredItems = useMemo(() => {
    const filtered = items.filter(item => {
      ...
      // 3. Drawer Filters
      if (filters.segments.length > 0 && !filters.segments.includes(item.segment)) return false;
      if (filters.statuses.length > 0 && !filters.statuses.includes(item.status)) return false;
      if (filters.budgetMin && (item.budgetMax || 0) < parseInt(filters.budgetMin, 10)) return false;
      if (filters.budgetMax && (item.budgetMin || Infinity) > parseInt(filters.budgetMax, 10)) return false;
      
      // Filter showClosed
      if (!filters.showClosed && (item.status === 'EXPIRED' || item.status === 'LOST' || item.status === 'WON')) {
        // If they explicitly clicked a closed tab, let it override
        if (activeTab !== 'EXPIRED' && activeTab !== 'WON' && activeTab !== 'LOST') {
          return false;
        }
      }
  ...
  if (user.businesses?.[0]?.kycStatus !== 'VERIFIED') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center max-w-md mx-auto px-6">
        ...
        <h2 className="text-xl font-bold text-text-primary mb-2">RFQ ke liye KYC zaroori hai</h2>
        <p className="text-sm text-text-secondary mb-8">Buyers ke saath deal karne ke liye pehle verification complete karein.</p>
        <a 
          href="/settings#kyc" 
        ...
        >
          KYC Complete Karein <span aria-hidden="true" className="ml-2">→</span>
        </a>
      </div>
    );
  }

  const hasAnyFilters = searchQuery.length > 0 || filters.segments.length > 0 || filters.statuses.length > 0 || filters.budgetMin !== '' || filters.budgetMax !== '';
  ...
  return (
    <div className="space-y-6">
      ...
      {expiringCount > 0 && activeTab !== 'QUOTED' && (
        <div className="bg-error-50 border-l-4 border-error-500 p-4 rounded-r-md flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3 text-error-700">
            ...
            <p className="text-sm font-medium">⏰ {expiringCount} RFQs 2 ghante mein expire ho rahe hain — jaldi quote karein!</p>
          </div>
          <button 
            type="button"
            onClick={() => {
      ...
      {permissions.isSuspended && (
         <ErrorBanner 
           message="Aapka account suspended hai. Aap nayi quote submit nahi kar sakte, sirf purani quote dekh sakte hain." 
         />
      )}
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`inline-flex items-center px-3 py-2 border shadow-sm text-sm font-medium rounded-md transition-colors ${
              sortByExpiry ? 'border-brand-300 text-brand-700 bg-brand-50' : 'border-neutral-300 text-text-primary bg-surface-base hover:bg-neutral-50'
            }`}
          >
      ...
      {hasAnyFilters && (
        <div className="flex flex-wrap items-center gap-2">
          {filters.segments.map(seg => (
            <span key={seg} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700">
              Segment: {seg.replace(/_/g, ' ').toLowerCase()}
              <button type="button" onClick={() => setFilters(f => ({...f, segments: f.segments.filter(s => s !== seg)}))} className="text-neutral-500 hover:text-neutral-700">×</button>
            </span>
          ))}
          {filters.statuses.map(st => (
            <span key={st} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700">
              Status: {st}
              <button type="button" onClick={() => setFilters(f => ({...f, statuses: f.statuses.filter(s => s !== st)}))} className="text-neutral-500 hover:text-neutral-700">×</button>
            </span>
          ))}
          {filters.budgetMin && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700">
              ...
              <button type="button" onClick={() => setFilters(f => ({...f, budgetMin: ''}))} className="text-neutral-500 hover:text-neutral-700">×</button>
            </span>
          )}
          {filters.budgetMax && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700">
              Max ₹{filters.budgetMax}
              <button type="button" onClick={() => setFilters(f => ({...f, budgetMax: ''}))} className="text-neutral-500 hover:text-neutral-700">×</button>
            </span>
          )}
          {filters.expiry && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700">
              Expiry: {filters.expiry}
              <button type="button" onClick={() => setFilters(f => ({...f, expiry: ''}))} className="text-neutral-500 hover:text-neutral-700">×</button>
            </span>
          )}
          {filters.showClosed && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700">
              Closed Included
              <button type="button" onClick={() => setFilters(f => ({...f, showClosed: false}))} className="text-neutral-500 hover:text-neutral-700">×</button>
            </span>
          )}
          <button 
            type="button" 
            onClick={() => {
              setSearchQuery('');
              setFilters({ segments: [], statuses: [], budgetMin: '', budgetMax: '', expiry: '', showClosed: false });
            }}
      ...
      {filteredItems.length > 0 || loading ? (
        <RfqTable 
          items={filteredItems} 
          isLoading={loading} 
          canQuote={permissions.canSubmitRfqQuote && !permissions.isSuspended} 
        />
      ) : (
        <RfqEmptyState 
          ...
          onClearFilters={() => {
            setSearchQuery('');
            setFilters({
              segments: [], statuses: [], budgetMin: '', budgetMax: '', expiry: '', showClosed: false
            });
          }} 
        />
      )}
