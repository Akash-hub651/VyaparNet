...
export function SkeletonText({
  lines = 1,
  className = '',
}: { lines?: number; className?: string }): React.JSX.Element {
  return (
...
export function SkeletonTable({
  rows = 8,
  cols = 5,
}: { rows?: number; cols?: number }): React.JSX.Element {
  return (
    <div aria-label="Loading..." aria-busy="true" role="status">
      ...
      <div className="flex items-center gap-4 px-4 py-2.5 bg-surface-app border-b border-border-default">
        ...
        {Array.from({ length: cols - 1 }).map((_, i) => (
          <Skeleton key={i} className="h-3 rounded" style={{ flex: i === 0 ? 2 : 1 }} />
        ))}
      </div>
      {/* Rows */}
...
export function SkeletonDetailPage(): React.JSX.Element {
  return (
    <div aria-label="Loading..." aria-busy="true" role="status" className="space-y-6">
      {/* Page header */}
    ...
    >
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="w-12 h-12 rounded-xl bg-brand-600 flex items-center justify-center">
          <span className="text-white text-lg font-bold tracking-tight">VN</span>
        </div>
        {/* Shimmer bar */}
        <div className="w-40 h-1.5 rounded-full overflow-hidden bg-neutral-200">
          <div className="h-full skeleton rounded-full" style={{ width: '60%' }} />
        </div>
