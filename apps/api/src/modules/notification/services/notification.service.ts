...
export class NotificationService {
  ...
  async createAndEnqueue(input: CreateAndEnqueueInput): Promise<void> {
    ...
    this.logger.debug(
      { userId, eventType, templateName },
      'NOTIFICATION_INAPP_CREATED',
    );
    ...
    if (
      ...
      this.preferenceService.isChannelEnabled(prefs, 'sms', category)
    ) {
    ...
    if (
      ...
      this.preferenceService.isChannelEnabled(prefs, 'email', category)
    ) {
    ...
    if (
      ...
      this.preferenceService.isChannelEnabled(prefs, 'push', category)
    ) {
    ...
    await Promise.allSettled(
      jobs.map((job) =>
        this.notificationsQueue.add(`deliver-${job.channel}`, job, {
  ...
  async sendDirect(
    ...
    if (!template) {
      this.logger.error(
        { templateName, userId },
        'SEND_DIRECT_TEMPLATE_NOT_FOUND',
      );
