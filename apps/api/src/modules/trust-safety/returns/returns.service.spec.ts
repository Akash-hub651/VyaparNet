...
describe('ReturnsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      ...
      returnRequest: { findFirst: vi.fn(), create: vi.fn(), findMany: vi.fn(), findUnique: vi.fn() },
  ...
  describe('validateReturnTransition', () => {
    it('should pass valid transition (STATE MACHINE)', () => {
      expect(() => service.validateReturnTransition(ReturnStatus.PENDING, ReturnStatus.APPROVED_FOR_PICKUP)).not.toThrow();
    });

    it('should reject invalid transition (STATE MACHINE)', () => {
      expect(() => service.validateReturnTransition(ReturnStatus.PENDING, ReturnStatus.QC_APPROVED)).toThrow(BadRequestException);
    });
  });
  ...
  describe('validateReturnEligibility', () => {
    ...
    it('should allow valid eligibility (HAPPY PATH)', async () => {
      ...
      const res = await service.validateReturnEligibility('ord_1', 'item_1', 'buyer_1');
      expect(res.order.id).toBe('ord_1');
    });

    it('should reject wrong actor (OWNERSHIP BOUNDARY)', async () => {
      prisma.order.findUnique.mockResolvedValue(mockOrder);
      await expect(service.validateReturnEligibility('ord_1', 'item_1', 'buyer_2')).rejects.toThrow(ForbiddenException);
    });

    it('should reject PROCESSING order', async () => {
      prisma.order.findUnique.mockResolvedValue({ ...mockOrder, status: OrderStatus.PROCESSING });
      await expect(service.validateReturnEligibility('ord_1', 'item_1', 'buyer_1')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('createReturnRequest', () => {
    it('should create return request atomically (FINANCIAL INTEGRITY)', async () => {
      ...
      expect(prisma.returnRequest.create).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({ requestedRefundAmount: new Prisma.Decimal('100.50') }),
      }));
