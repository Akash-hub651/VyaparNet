...
import { CreateRfqDto, CreateQuotationDto, NegotiatePriceDto, RfqConvertDto, SystemActorType } from '@vyaparnet/types';
...
export class RfqService {
  ...
  async createRfq(buyerId: string, dto: CreateRfqDto) {
    ...
    // Metric for RFQ not available in MetricsService yet
    return rfq;
  }
  ...
  async acceptQuotation(rfqId: string, quotationId: string, buyerId: string) {
    ...
    if (quotation.status !== QuotationStatus.NEGOTIATING && quotation.status !== QuotationStatus.DRAFT) {
      throw new BadRequestException('Quotation is not in a valid state to be accepted');
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const accepted = await this.quotationRepo.updateStatus(quotationId, QuotationStatus.ACCEPTED_BY_BUYER, tx);
      // Close other quotes
      await tx.quotation.updateMany({
        where: { rfqId, id: { not: quotationId }, status: { not: QuotationStatus.EXPIRED } },
  ...
  async convertToOrder(_rfqId: string, quotationId: string, buyerId: string, dto: RfqConvertDto, ipAddress: string) {
    return this.ordersService.createFromQuotation(quotationId, buyerId, {
      ...dto,
  ...
  async getSellerRfqs(sellerId: string) {
    ...
    const business = await this.prisma.business.findFirst({ where: { ownerId: sellerId } });
    if (!business) return [];
  ...
  async submitQuotation(rfqId: string, sellerId: string, dto: CreateQuotationDto) {
    // KYC Gate (INV-S8-27)
    const business = await this.prisma.business.findFirst({ where: { ownerId: sellerId } });
    ...
    const quotation = await this.prisma.$transaction(async (tx) => {
      const q = await this.quotationRepo.create({
        ...
        requestedPrice: item.requestedPrice ? new Prisma.Decimal(item.requestedPrice) : null,
      })), tx);
  ...
  async negotiatePrice(quotationId: string, userId: string, actorType: 'BUYER' | 'SELLER', dto: NegotiatePriceDto) {
    let quotation;
    if (actorType === 'BUYER') {
      quotation = await this.quotationRepo.findByIdForBuyer(quotationId, userId);
    } else {
      ...
      quotation = await this.quotationRepo.findByIdForSeller(quotationId, business.id);
    }

    if (!quotation) throw new NotFoundException('Quotation not found');
    if (quotation.status !== QuotationStatus.DRAFT && quotation.status !== QuotationStatus.NEGOTIATING) {
    ...
    const configRow = await this.prisma.appConfig.findUnique({ where: { key: 'MAX_NEGOTIATION_ROUNDS' } });
    const maxRounds = configRow ? parseInt(configRow.value, 10) : 3;

    if (quotation.negotiations.length >= maxRounds) {
      throw new BadRequestException(`Maximum negotiation rounds (${maxRounds}) exceeded`);
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (quotation.status === QuotationStatus.DRAFT) {
        await this.quotationRepo.updateStatus(quotation.id, QuotationStatus.NEGOTIATING, tx);
      }
      return neg;
    });
