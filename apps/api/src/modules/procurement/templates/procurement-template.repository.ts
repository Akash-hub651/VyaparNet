...
export class ProcurementTemplateRepository {
  ...
  async create(data: Prisma.ProcurementTemplateUncheckedCreateInput): Promise<ProcurementTemplate> {
  ...
  async findByIdForBuyer(id: string, buyerId: string): Promise<ProcurementTemplate | null> {
    return this.prisma.procurementTemplate.findFirst({
      where: { id, buyerId, isDeleted: false },
    });
  }

  async update(id: string, data: Prisma.ProcurementTemplateUpdateInput): Promise<ProcurementTemplate> {
