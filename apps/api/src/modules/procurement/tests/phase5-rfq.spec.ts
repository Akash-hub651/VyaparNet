...
describe('Phase 5: ProcurementModule + RFQ (INV-S8)', () => {
  ...
  describe('INV-S8-17: Seller Quote Scoping', () => {
    it('should only return RFQs matching seller segment and products', async () => {
      ...
      expect(prisma.business.findFirst).toHaveBeenCalledWith({
        where: { ownerId: 'seller-1' },
      });
      expect(prisma.product.findMany).toHaveBeenCalledWith({
        where: { businessId: 'biz-1', isDeleted: false },
        select: { id: true },
      });
      expect(rfqRepo.findManyForSeller).toHaveBeenCalledWith('ELECTRONICS', [
        'prod-1',
        'prod-2',
      ]);
    ...
    it('should return empty array if seller has no products in catalog (INV-S8-17)', async () => {
      prisma.business.findFirst.mockResolvedValue({
        id: 'biz-1',
        segment: 'TEXTILE',
      } as any);
  ...
  describe('INV-S8-27: KYC Validation for Quotes', () => {
    it('should throw ForbiddenException if seller is not VERIFIED', async () => {
      ...
      await expect(
        service.submitQuotation('rfq-1', 'seller-1', {} as any),
  ...
  describe('INV-S8-20: Negotiation Rounds Limit', () => {
    it('should throw BadRequestException if max rounds exceeded', async () => {
      ...
      await expect(
        service.negotiatePrice('quote-1', 'seller-1', 'SELLER', {
          proposedPrice: '90.00',
          message: 'Counter',
        }),
    ...
    it('should add negotiation if rounds are within limit', async () => {
      ...
      const result = await service.negotiatePrice(
        'quote-1',
        'seller-1',
        'SELLER',
