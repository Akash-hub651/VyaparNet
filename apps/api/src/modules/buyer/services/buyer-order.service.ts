...
import { BuyerOrderRepository, BuyerOrderDetailShape } from '../repositories/buyer-order.repository';
...
export class BuyerOrderService {
  ...
  async getOrders(buyerId: string, filter: { status?: OrderStatus; cursor?: string; limit: number }) {
    return this.buyerOrderRepo.findManyForBuyer(buyerId, filter);
  }

  async getOrder(orderId: string, buyerId: string): Promise<BuyerOrderDetailShape> {
    return this.buyerOrderRepo.findByIdForBuyer(orderId, buyerId);
  }

  async cancelOrder(orderId: string, buyerId: string, reason: string): Promise<{ id: string; status: string }> {
    ...
    if (order.status === 'CONFIRMED') {
      ...
      const confirmedAtMs = order.confirmedAt ? new Date(order.confirmedAt).getTime() : 0;
    ...
    if (order.status === 'PLACED' || order.status === 'CONFIRMED') {
      try {
        ...
        this.logger.error({ orderId, buyerId, error: (releaseErr as Error).message }, 'INVENTORY_RELEASE_FAILED_AFTER_CANCEL');
