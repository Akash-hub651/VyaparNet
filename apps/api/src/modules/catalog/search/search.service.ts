...
import { SEARCH_ENGINE, SearchEngine, SearchResult, SearchSuggestionResponse } from './postgres-search.engine';
...
export class SearchService {
  ...
  async searchProducts(dto: SearchProductsDto, _userId?: string, scope: 'buyer' | 'admin' = 'buyer') {
    ...
    const cacheKey = this.cacheService.buildKey(scope, dto.segment, normalized, filters);
    ...
    if (results.length === 0 && dto.q) {
      ...
      results = await this.searchEngine.fuzzyFallback(dto.q, dto.segment, limit);
    }
  ...
  async getSuggestions(query: string, segment: Segment): Promise<SearchSuggestionResponse> {
  ...
  private async applySegmentBoosting(segment: Segment, results: SearchResult[]): Promise<SearchResult[]> {
    ...
    try {
      ...
      this.logger.error(`Failed to parse segment boosting config for ${segment}`, e);
      return results;
    }
  }

  private logSearchAnalytics(query: string, segment: Segment, resultCount: number) {
    // Fire and forget analytics logging
    // AppConfig sample rate can limit noise.
    const sampleRate = this.configService.get<number>('SEARCH_ANALYTICS_SAMPLE_RATE') || 0.1;
    if (Math.random() < sampleRate) {
      this.logger.debug(`[Analytics] Search logged: "${query}" in ${segment} yielded ${resultCount} results.`);
    }
  }
}
