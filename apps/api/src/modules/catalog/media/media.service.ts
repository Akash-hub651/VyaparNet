...
import { STORAGE_SERVICE_TOKEN, StorageService } from './storage/storage.service.interface';
...
export class MediaService {
  ...
  async upload(
    ...
    // 5. Check duplicate
    const existing = await this.mediaRepository.findByChecksum(checksum, userId);
    if (existing) {
