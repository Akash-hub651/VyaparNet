...
export class SellerOrderService {
  ...
  async getOrder(
    ...
    const order = await this.sellerOrderRepo.findByIdForSeller(
      orderId,
      seller.businessId,
    );
  ...
  async transitionStatus(
    ...
    if (dto.toStatus === 'SHIPPED') {
      if (!dto.trackingNumber?.trim()) {
        throw new UnprocessableEntityException({
          ...
          message: 'Tracking number is required when transitioning to SHIPPED',
        });
      }
    }
    ...
    if (isNew === null) {
      ...
      const current = await this.sellerOrderRepo.findByIdForSeller(
        orderId,
        seller.businessId,
      );
    ...
    const timestampField =
      STATUS_TIMESTAMP_FIELD_MAP[dto.toStatus as OrderStatus];
    ...
    if (dto.toStatus === 'SHIPPED' && order.confirmedAt) {
      const dispatchHours =
        (Date.now() - order.confirmedAt.getTime()) / 3600000;
      this.metrics.sellerDispatchTimeHours.observe(
        { segment: seller.segment },
        dispatchHours,
      );
    }
    ...
    await this.redis
      .del(`kpi:${seller.businessId}:${seller.segment}:${today}`)
      .catch(() => {});
    ...
    this.logger.log({
      ...
      statusTo: dto.toStatus,
      trackingNumber: dto.trackingNumber ?? null,
      durationFromConfirmedMs: order.confirmedAt
        ? Date.now() - order.confirmedAt.getTime()
        : null,
      segment: seller.segment,
    });

    const updated = await this.sellerOrderRepo.findByIdForSeller(
      orderId,
      seller.businessId,
    );
